...
export class ModalGuideComponent {
    ...
    updateContent(protocol, guideData) {
        // Ensure protocolGuides is available (global from config/protocols.js)
        if (typeof protocolGuides === 'undefined' && !guideData) return;

        const guide = guideData || protocolGuides[protocol] || protocolGuides["SISTEMA NOMINAL"];
