    ...
    init() {
        ...
        if (this.mobileContainer) {
            ...
            Object.keys(this.eggs).forEach(key => this.createButton(key, this.mobileContainer, () => {
                window.dispatchEvent(new CustomEvent('request-close-mobile-menu', { detail: { menu: 'easter-eggs' } }));
            }));
        }
    }
    ...
    createButton(key, container, onSuccess) {
        ...
        btn.onclick = () => {
            ...
            if (window.innerWidth < 768) {
                window.dispatchEvent(new Event('request-show-outputs'));
            }
